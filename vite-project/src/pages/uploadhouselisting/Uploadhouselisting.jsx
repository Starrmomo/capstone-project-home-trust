...
export default function AddPropertyCombined() {
 ...
 const handleSubmit = async () => {
  ...
  // Keep your size validation
  const totalBytes =
    images.reduce((sum, f) => sum + (f?.size || 0), 0) +
    (utilityBillFile?.size || 0);

  if (totalBytes > MAX_TOTAL_SIZE_BYTES) {
    setFileSizeWarning(
      `Total files are too large (max ${MAX_TOTAL_SIZE_MB}MB). Use smaller images (under ${MAX_IMAGE_SIZE_MB}MB each).`
    );
    alert(
      `Files are too large. Please keep property images under ${MAX_IMAGE_SIZE_MB}MB each and total under ${MAX_TOTAL_SIZE_MB}MB.`
    );
    return;
  }
  ...
  try {
    ...
    if (result.status === "verified") {
      ...
      setStatus("error");
      alert("Property could not be verified.");
    }
  } catch (error) {
    console.error(error);
    setStatus("error");

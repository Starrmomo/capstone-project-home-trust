...
export default function Landingpage() {
  ...
  // ----------------------------
  const dummyListings = [
    { id: 1, name: "3 Bedroom House", location: "Ikoyi", price: "₦200,000" },
    { id: 2, name: "2 Bedroom Apartment", location: "Lekki", price: "₦150,000" },
    { id: 3, name: "Studio Flat", location: "Victoria Island", price: "₦100,000" },
  ];
  ...
  const fetchListings = () => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
      navigate("/search", { state: { listings: dummyListings } });
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.mainButtons}>
        <button className={styles.primary} onClick={fetchListings} disabled={loading}>
          {loading ? "Loading..." : <><Searchicon /> Find a Home</>}
        </button>
      ...
      <h3 className={styles.sectionTitle}>Why trust us?</h3>
      <div className={styles.iconNav}>
        <div onClick={() => navigate("/landingpage")}><Houseicon /><span>Home</span></div>
        <div onClick={() => navigate("/search")}><Searchicon2 /><span>Search</span></div>
        <div onClick={() => navigate("/saved")}><Savedicon /><span>Saved</span></div>
        <div onClick={() => navigate("/chat")}><Chat /><span>Chat</span></div>
        <div onClick={() => navigate("/tenantprofile")}><Profile /><span>Profile</span></div>
      </div>
